...
namespace Genetic_Algorythm
{
    public partial class Form2 : Form
    {
        ...
        {
            InitializeComponent();
            MessageBox.Show("Реализация генетического алгоритма для нахождения оптимального решения задачи о ранце." + Environment.NewLine + "Решение доступно для 5, 10, 15 и 20 элементов с заранее заданными весами и значениями. Тем не менее, число популяции и количество поколений настроены, а также кроссовер и мутация вероятности." + Environment.NewLine + "Решение может быть выполнено в двух режимах: шаг за шагом с выходом текущей популяции на каждом шагу, и автоматический расчет с выходом конечной популяции и лучшего решения.", "Информация", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (populationSize < 100)
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.IsNewRow)
                        continue;
                    row.HeaderCell.Value = "Хромосома " + rowNumber;
            ...
            {
                column.HeaderText = String.Concat("Элемент ",
                    (column.Index + 1).ToString());
            }
            dataGridView2.Rows[0].HeaderCell.Value = " Значение";
            dataGridView2.Rows[1].HeaderCell.Value = "Вес";
            for (int i = 0; i < numberOfItems; i++)
            {
            ...
            calculateFitness();
            label15.Text = "Хромосома " + (bestSolutionIndex + 1) + ", значение: " + bestSolutionValue + ", " + Environment.NewLine + "вес: " + bestSolutionWeight + ", " + bestSolutionQuantity + " элементов взято.";
            label14.Text = "Хромосома " + (bestFitnessIndex + 1) + ", значение: " + bestFitnessValue + ", вес: " + bestFitnessWeight + ", " + bestFitnessQuantity + " элементов взято";
            currentGeneration++;
        ...
        {
            label17.Text = (currentGeneration).ToString();
            label10.Text = "Текущая популяция:";
            for (int i = 0; i < numberOfItems; i++)
                for (int j = 0; j < populationSize; j++)
            ...
            {
                label10.Text = "Конечная популяция:";
                button2.Visible = !button2.Visible;
                button3.Visible = !button3.Visible;
